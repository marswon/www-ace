...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		var p = $.extend(true,{
			...
			ExpandColumn: null,
			tree_root_level : 0,
			prmNames: {page:"page",rows:"rows", sort: "orderBy",order: "sord", search:"_search", nd:"nd", id:"id",oper:"oper",editoper:"edit",addoper:"add",deloper:"del", subgridid:"id", npage: null, totalrows:"totalrows"},
			forceFit : false,
			gridstate : "visible",
			cellEdit: false,
			cellsubmit: "remote",
		...
		addCell = function(rowId,cell,pos,irow, srvr, rdata) {
			...
			tmp=v;
			if(ts.p.colModel[pos].renderer){
				v=ts.p.colModel[pos].renderer(v,srvr);
			}
			prp = formatCol( pos,irow, tmp, srvr, rowId, rdata);
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			while (i<len) {
				...
				for (j=0;j<rowReader.length;j++) {
					//console.log(rowReader[j]);
					v = $.jgrid.getAccessor(cur,rowReader[j]);
					rd[ts.p.colModel[j+gi+si+ni].name] = v;
					//console.log(addCell(idr,v,j+gi+si+ni,i+rcnt,cur, rd));
		...
		populate = function (npage) {
			if(!ts.grid.hDiv.loading) {
				...
				//分页自定义参数
				if(pN.totalRecord !== null) {prm[pN.totalRecord]= ts.p.records;}
				if(pN.start !== null) {prm[pN.start]=(ts.p.page-1)*ts.p.rowNum;}
				//结束
					...
					$.ajax($.extend({
						...
						success:function(data,st, xhr) {
							...
							if(dt === "xml") { 
								...
								//根据业务返回的状态判断是否成功
								if(!data.state){
									$.jgrid.info_dialog($.jgrid.errors.info,data.errorMessage,$.jgrid.edit.bClose,null,data.detail);
								}else{
...
"use strict";	
	...
	$.unformat = function (cellval,options,pos,cnt) {
		...
		//console.log($(cellval).attr('title'));
		return ret !== undefined ? ret : cnt===true ? $(cellval).text() : $.jgrid.htmlDecode($(cellval).attr('title'));
	};
...
//Helper functions
	...
	info_dialog : function(caption, content,c_b, modalopt,detail) {
		...
		if(detail&&detail!=''){
			cnt+="<div style='text-align:center'><textarea cols=55 rows=4>"+detail+"</textarea></div>";
		}
...
$.jgrid.extend({
	...
	editGridRow : function(rowid, p){
				...
				if ( nm !== 'cb' && nm !== 'subgrid' && this.editable===true && nm !== 'rn') {
					if( $.inArray(this.edittype, ['combotree','combogrid','datetimebox','datebox','combobox']) > -1) {
						switch (this.edittype) {
							case "combogrid":
								if(this.dataoptions){
									if(this.dataoptions.url&&p.selrow){
										//var r=$($t).jqGrid("getRowData",p.selrow);
										//console.log(r);
										if(this.dataoptions.url.indexOf("?")>-1){
											this.dataoptions.url=this.dataoptions.url.split("?")[0]+"?id="+vl;
										}else{
											this.dataoptions.url=this.dataoptions.url+"?id="+vl
										}
		...
		return this.each(function(){
			...
			showFrm = true,
			maxCols = 2, maxRows=0,	postdata, diff, frmoper;
							...
							switch (this.edittype) {
								...
								case "combotree":
									...
									if(tmp&&tmp.length>1){
										...
										val=$(_id).combotree('getValue');
									}
								break;
								case "datebox":
									val=$(_id).datebox('getValue');
								break;
								case "datetimebox":
									val=$(_id).datetimebox('getValue');
								break;
								case "combobox":
			...
			function createData(rowid,obj,tb,maxcols){
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && this.editable===true && nm !== 'rn') {
						...
						if(scl){
							...
							if(opt.colspan){
								...
								$("td:eq(2)",trdata[0]).remove();
								$("td:eq(3)",trdata[0]).remove();
                                t++;
								cnt++;
				...
				/*$(tb).find('.FormData').each(function(){
					console.log(this);
				});*/
				if( cnt > 0) {
			...
			function postIt() {
				...
				if(ret[0] && !rp_ge[$t.p.id].processing) {
					...
					var _operurl=rp_ge[$t.p.id].url || $($t).jqGrid('getGridParam','editurl');
					if(postdata[oper]=='add'){
						_operurl=rp_ge[$t.p.id].url || $($t).jqGrid('getGridParam','addurl');
					}
					if(postdata[oper]=='edit'){
						_operurl=rp_ge[$t.p.id].url || $($t).jqGrid('getGridParam','editurl');
					}
					...
					var ajaxOptions = $.extend({
						...
						data:{jsons:JSON.stringify($.isFunction(rp_ge[$t.p.id].serializeEditData) ? rp_ge[$t.p.id].serializeEditData.call($t,postdata) :  postdata)},
						complete:function(data,status){
							console.log(JSON.stringify($.isFunction(rp_ge[$t.p.id].serializeEditData) ? rp_ge[$t.p.id].serializeEditData.call($t,postdata) :  postdata));
							//console.log("ajax responsed");
							if(aftersavefunc){
								aftersavefunc(data);
							}
							var key;
							...
							if(data.status >= 300 && data.status !== 304) {
								...
								//根据业务返回的状态判断是否成功
								var rst=JSON.parse(data.responseText);
								
								if(!rst.state){
									ret = [false,rst.errorMessage,""];
								}else{
									...
									//$.jgrid.info_dialog($.jgrid.errors.info,rst.errorMessage);
									//$.jgrid.info_dialog($.jgrid.errors.info,rst.errorMessage,$.jgrid.edit.bClose);
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				$("#dData","#"+dtbl+"_2").click(function(){
					...
					if(ret[0] && !rp_ge[$t.p.id].processing) {
						...
						var _operurl=rp_ge[$t.p.id].url || $($t).jqGrid('getGridParam','deleteurl');
						var aftersavefunc=$($t).jqGrid('getGridParam','aftersavefunc');
						console.log(_operurl);
						var ajaxOptions = $.extend({
							url: _operurl,
							type: rp_ge[$t.p.id].mtype,
							data: {jsons:JSON.stringify($.isFunction(rp_ge[$t.p.id].serializeDelData) ? rp_ge[$t.p.id].serializeDelData.call($t,postd) : postd)},
							complete:function(data,status){
								console.log(JSON.stringify($.isFunction(rp_ge[$t.p.id].serializeDelData) ? rp_ge[$t.p.id].serializeDelData.call($t,postd) : postd));
								if(aftersavefunc){
									aftersavefunc(data);
								}
								...
								$.jgrid.info_dialog($.jgrid.errors.info,rst.errorMessage||ret[1],$.jgrid.edit.bClose);
							}
...
//Editing
	...
	saveRow : function(rowid, successfunc, url, extraparam, aftersavefunc,errorfunc, afterrestorefunc) {
		...
		var success = false;
		var $t = this[0], nm, tmp={}, tmp2={}, tmp3= {}, editable, fr, cv, ind,_id;
			...
			$('td[role="gridcell"]',ind).each(function(i) {
				...
				if ( nm !== 'cb' && nm !== 'subgrid' && cm.editable===true && nm !== 'rn' && !$(this).hasClass('not-editable-cell')) {
					...
					case "combotree":
						...
						tmp2[nm]=$(_id).combotree('getText');
					break;
					case "datebox":
						tmp[nm]=$(_id).datebox('getValue');
						tmp2[nm]=$(_id).datebox('getText');
					break;
					case "datetimebox":
...
$.jgrid.extend({
	editCell : function (iRow,iCol, ed){
		return this.each(function (){
			var $t = this, nm, tmp,cc, cm,_id,vl='';
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				var r=$($t).jqGrid("getRowData",$t.p.selrow);;
				if(r&&r[nm]){
					vl=r[nm];
				}
				console.log(vl);
				...
				if( $.inArray(cm.edittype, ['combotree','combogrid','datetimebox','datebox','combobox']) > -1) {
					switch (cm.edittype) {
						case "combogrid":
							if(cm.dataoptions){
								if(cm.dataoptions.url&&$t.p.selrow){
									if(cm.dataoptions.url.indexOf("?")>-1){
										cm.dataoptions.url=cm.dataoptions.url.split("?")[0]+"?id="+vl;
									}else{
										cm.dataoptions.url=cm.dataoptions.url+"?id="+vl
									}
									console.log(cm.dataoptions.url);
								}
							}
			...
			if(fr !== null) {
				...
				case "combogrid":
					...
					if(tmp&&tmp.length>1){
						...
						v=$(_id).combogrid('getValue');
					}
					v2=$(_id).combogrid('getText');
				break;
				case "combotree":
					var tmp=$(_id).combotree('getValues');
					if(tmp&&tmp.length>1){
						...
						v=$(_id).combotree('getValue');
					}
					v2=$(_id).combotree('getText');
				break;
				case "datebox":
					v=$(_id).datebox('getValue');
					v2=$(_id).datebox('getText');
				break;
				case "datetimebox":
	 ...
	 columnChooser : function(opts) {
			...
			opts = $.extend({
				...
				apply_perm : function() {
					...
					self.jqGrid('setGridWidth', $(".page-content").width(),shrinkToFit);
