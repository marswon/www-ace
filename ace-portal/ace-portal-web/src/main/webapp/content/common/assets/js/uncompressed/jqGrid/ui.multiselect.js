...
$.widget("ui.multiselect", {
  options: {
	...
	_create: function() {
		...
		// fix list height to match <option> depending on their individual
		// header's heights
...
//		 $("#colchooser_grid-table").find("ul").click(function(){
//			 setTimeout(function(){
//					$("#colchooser_grid-table").find("li").each(function(){
...
//		});
			...
			if ( direction ) {
				while (i>=0 && i<items.length) {
					...
					if ( direction != comparator(item, $(items[i])) ) {
						// going up, go back one item down, otherwise leave as
						// is
	...
	_applyItemState: function(item, selected) {
		item.find('a.action span').addClass('ace-icon fa')
		if (selected) {
			if (this.options.sortable)
				item.children('span').addClass('ui-icon-arrowthick-2-n-s').removeClass('ui-helper-hidden').addClass('ui-icon');
			else
				item.children('span').removeClass('ui-icon-arrowthick-2-n-s').addClass('ui-helper-hidden').removeClass('ui-icon');
			item.find('a.action span').addClass('fa-minus-circle').removeClass('fa-plus-circle');
			this._registerRemoveEvents(item.find('a.action'));
			
		} else {
			item.children('span').removeClass('ui-icon-arrowthick-2-n-s').addClass('ui-helper-hidden').removeClass('ui-icon');
			item.find('a.action span').addClass('fa-plus-circle').removeClass('fa-minus-circle');
	...
	_registerDoubleClickEvents: function(elements) {
		...
		elements.dblclick(function(ev) {
			...
			if ($(ev.target).closest('.action').length === 0) {
				// This may be triggered with rapid clicks on actions as well.
				// In that
