...
function initModel(){
	...
	if (customActivityColors !== null && customActivityColors !== undefined && customActivityColors.length > 0) {
	 ...
	 customActivityToolTips = modelDiv.attr('data-activity-tooltips');
	...
	// Support for custom opacity for activity backgrounds
	 customActivityBackgroundOpacity = modelDiv.attr('data-activity-opacity');
}
...
var request = {};
function buidModel(instId){
	if(instId=='12525'){
		instId='15023';	
	}
	initModel();
	var modelUrl=contextPath+'/workflow/getModelJSON.do?processInstanceId='+instId;
	request = jQuery.ajax({
	    type: 'get',
