...
public class WebConfigurer implements ServletContextListener {
	...
	private void initSpring(ServletContext servletContext,
		...
		appDispatcherServletConfiguration
				.register(AppDispatcherServletConfiguration.class);

		log.debug("Registering Spring MVC Servlet");
		ServletRegistration.Dynamic appDispatcherServlet = servletContext
				.addServlet("appDispatcher", new DispatcherServlet(
						appDispatcherServletConfiguration));
		...
		apiDispatcherServletConfiguration
				.register(ApiDispatcherServletConfiguration.class);

		ServletRegistration.Dynamic apiDispatcherServlet = servletContext
				.addServlet("apiDispatcher", new DispatcherServlet(
						apiDispatcherServletConfiguration));
	...
	private void initSpringSecurity(ServletContext servletContext,
		...
		log.info("===================Registering Spring Security Filter==========================");
		String beans[] = this.context.getBeanDefinitionNames();
		int i = 0;
		for (String e : beans) {
			log.info("init bean {} at {}", i, e);
			i++;
		}
		DelegatingFilterProxy springSessionRepositoryFilter = new DelegatingFilterProxy("springSessionRepositoryFilter");
		springSessionRepositoryFilter.setServletContext(servletContext);
		
		
		FilterRegistration.Dynamic filter = servletContext.addFilter("springSessionRepositoryFilter", springSessionRepositoryFilter);
		filter.addMappingForUrlPatterns(null, false, "/*");
		filter.setAsyncSupported(true);
		
		WebAccessDecisionSecurityFilter webAccessDecisionSecurityFilter=new WebAccessDecisionSecurityFilter();
		@SuppressWarnings("unchecked")
		RedisOperations<String, String> redisTemplateString = (RedisOperations<String, String>) this.context.getBean("redisTemplateString");
		webAccessDecisionSecurityFilter.setRedisTemplateString(redisTemplateString);;
		FilterRegistration.Dynamic filter2 = servletContext.addFilter("webAccessDecisionSecurityFilter", webAccessDecisionSecurityFilter);
		filter2.addMappingForUrlPatterns(null, false, "/*");
		filter2.setAsyncSupported(true);
	}
